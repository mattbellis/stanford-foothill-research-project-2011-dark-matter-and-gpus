...
int main(int argc, char **argv)  
{
	...
	while(true)
	{
		...
		acc[0][0] = Acceleration(mass[0], force);
		acc[1][0] = Acceleration(mass[1], force);
		...
		for (int i=0;i<NUM_PARTICLES;i++) 
		{
			for (int j=0;j<3;j++) 
			{
				...
				if (i==0) pos[i][j] += DistancePart(vel[i][j], vel_temp[j], time); 
				else      pos[i][j] -= DistancePart(vel[i][j], vel_temp[j], time); 
			}
		}
