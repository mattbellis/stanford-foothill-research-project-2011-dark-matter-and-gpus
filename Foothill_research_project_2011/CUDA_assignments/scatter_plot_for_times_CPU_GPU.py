    ...
    for row in infile:
        ...
        # Otherwise, it is data
	       ...
	       print row[0]
               ...
               y_pts[j].append(float(row[j+1]))

               if j>0:
	          x_pts[j+2].append(float(row[0]))
	          y_pts[j+2].append(float(row[j+1])/float(row[1]))

        i += 1
    
    myplots = []
    formatting = ['ro','bo','go','mo','co']
    leg_text = ["GPU(with hist)", "GPU(no hist)", "CPU(no hist)", "Range_CPU_GPU(no hist)", "Range_GPU(hist)_GPU(no_hist)"]
    for i in range(0,3):
...
#    legend(loc='upper right')
 ...
 # subplots[0].set_xlim(1.5e9)
   # subplots[0].set_ylim(1.5e9)
    fig1.legend((myplots[0], myplots[1], myplots[2]), (leg_text[0], leg_text[1], leg_text[2]), 'upper right',1)
